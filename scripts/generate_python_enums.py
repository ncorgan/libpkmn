...
REPLACEMENT_FILENAMES = dict(
    stat = "pokemon_stat",
    type = "pokemon_type"
)

def process_enum_header(header_path, output_dir):
    header = CppHeaderParser.CppHeader(header_path)
    for enum in header.enums:
        enum_name = enum["name"][2:]
        filename = "{0}.py".format(REPLACEMENT_FILENAMES.get(enum_name, enum_name))
    ...
    for root, _, files in os.walk(enum_dir):
        for filename in files:
            if (filename not in IGNORE_FILES) and filename.endswith(".hpp"):
                process_enum_header(os.path.join(root, filename), output_dir)
