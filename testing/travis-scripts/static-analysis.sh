...
find $REPO_TOPLEVEL/lib -name '*.[ch]pp' | xargs cppcheck --enable=performance,portability,warning --std=c++11 -I $REPO_TOPLEVEL/include --error-exitcode=1 --force --quiet --template='[{file}:{line}]: ({severity}: {id}) {message}'
#[ $? -ne 0 ] && exit 1
echo Running CppCheck on C files.
find $REPO_TOPLEVEL/lib -name '*.[ch]' | xargs cppcheck --enable=performance,portability,warning --std=c99 -I $REPO_TOPLEVEL/include --error-exitcode=1 --force --quiet --template='[{file}:{line}]: ({severity}: {id}) {message}'
#[ $? -ne 0 ] && exit 1
