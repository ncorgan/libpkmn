...
// Test loading a known .pkm file from outside LibPKMN.
TEST(pokemon_io_test, test_outside_pkm) {
    ...
    EXPECT_EQ("None", riolu->get_held_item());
    EXPECT_EQ("Sayuri", riolu->get_original_trainer_name());
    EXPECT_EQ(11536, riolu->get_original_trainer_public_id());
    EXPECT_EQ(15275, riolu->get_original_trainer_secret_id());
    EXPECT_EQ(1001073936, riolu->get_original_trainer_id());
    EXPECT_EQ("Female", riolu->get_original_trainer_gender());
    EXPECT_EQ(159, riolu->get_current_trainer_friendship());
