...
static const pkmn_pokemon_box2_t empty_pokemon_box =
{
    .game = NULL,
    .capacity = 0,
    ._internal = NULL
...
)
{
    ...
    error = pkmn_pokemon_box2_get_num_pokemon(
                box_ptr,
                &num_pokemon
            );
    TEST_ASSERT_EQUAL(PKMN_ERROR_NONE, error);
...
)
{
    ...
    if(generation == 1)
    {
        error = pkmn_pokemon_box2_set_name(
                    box_ptr,
                    "ABCDEFGH"
                );
        TEST_ASSERT_EQUAL(PKMN_ERROR_FEATURE_NOT_IN_GAME_ERROR, error);
...
)
{
    ...
    pkmn_pokemon2_t original_first = empty_pokemon;
    pkmn_pokemon2_t original_second = empty_pokemon;

    int generation = game_to_generation(box_ptr->game);

    error = pkmn_pokemon_box2_get_pokemon(
                box_ptr,
                0,
                &original_first
            );
    ...
    pkmn_pokemon2_t bulbasaur = empty_pokemon;
    pkmn_pokemon2_t charmander = empty_pokemon;
    pkmn_pokemon2_t squirtle = empty_pokemon;
    ...
    pkmn_pokemon2_t second_in_box = empty_pokemon;
    error = pkmn_pokemon_box2_get_pokemon(
                box_ptr,
                1,
                &second_in_box
    ...
    if(generation <= 2)
    {
        ...
        error = pkmn_pokemon2_free(&fifth_in_box);
        TEST_ASSERT_EQUAL(PKMN_ERROR_NONE, error);
...
)
{
    ...
    if(generation == 1)
    {
        error = pkmn_pokemon_pc2_get_box_names(
                    pc_ptr,
                    &box_names
                );
        TEST_ASSERT_EQUAL(PKMN_ERROR_FEATURE_NOT_IN_GAME_ERROR, error);
    ...
    else
    {
        ...
        {
            pkmn_pokemon_box2_t pokemon_box = empty_pokemon_box;

            error = pkmn_pokemon_pc2_get_box(
                        pc_ptr,
                        box_index,
                        &pokemon_box
                    );
            ...
            error = pkmn_pokemon_box2_free(&pokemon_box);
            TEST_ASSERT_EQUAL(PKMN_ERROR_NONE, error);
...
{
    pkmn_error_t error = PKMN_ERROR_NONE;
    pkmn_pokemon_box_list2_t pokemon_boxes = empty_pokemon_box_list;
    ...
    for(size_t box_index = 0; box_index < pokemon_boxes.length; ++box_index)
    {
        ...
        pkmn_pokemon2_t third_pokemon = empty_pokemon;
        ...
        error = pkmn_pokemon2_free(&third_pokemon);
        TEST_ASSERT_EQUAL(PKMN_ERROR_NONE, error);
        TEST_ASSERT_NULL(third_pokemon._internal);

        error = pkmn_pokemon2_free(&second_pokemon);
        TEST_ASSERT_EQUAL(PKMN_ERROR_NONE, error);
        TEST_ASSERT_NULL(second_pokemon._internal);

        error = pkmn_pokemon2_free(&first_pokemon);
        TEST_ASSERT_EQUAL(PKMN_ERROR_NONE, error);
        TEST_ASSERT_NULL(first_pokemon._internal);
    }

    error = pkmn_pokemon_box_list2_free(&pokemon_boxes);
    TEST_ASSERT_EQUAL(PKMN_ERROR_NONE, error);
...
)
{
    ...
    pkmn_error_t error = PKMN_ERROR_NONE;
    pkmn_pokemon_pc2_t pokemon_pc =
    {
        .game = NULL,
        ._internal = NULL
    };

    error = pkmn_pokemon_pc2_init(
                game,
                &pokemon_pc
            );
    TEST_ASSERT_EQUAL(PKMN_ERROR_NONE, error);
